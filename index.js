...
Do the following:   
   ...
   HINT: no function required
*/
let beggineggincheese = 1;
let beggineggs = .5;
if(beggineggincheese <= 1){
beggineggincheese + beggineggs;
}
...
function game(PC, CC){
  ...
  console.log(game("paper", randomChoice()));
...
function annoyingSong(sn){
        for (let a = sn; a > 1; a--){
          // return a + " bottles of soda on the wall, " + a + " bottles of soda, take one down pass it around " + a + " bottles of soda on the wall";
...
function vowelCounter(str) {
    ...
    for(let i = 0; i < str.length; i++){
      for(let a = 0; a < vowels.length; a++){
        if((str.charAt(i)).includes(vowels[a])){
          vn++; 
        }
      }
    }
...
console.log(vowelCounter('beggineggincheese'));
